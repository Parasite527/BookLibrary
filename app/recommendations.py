...
class BookRecommendationEngine:
    ...
    def get_user_reading_history(self, user_id: int) -> List[models.Book]:
        ...
        user_books = (
            ...
            .join(models.Borrow, models.Borrow.book_id == models.Book.id)
        ...
        reviewed_books = (
            self.db.query(models.Book)
            .join(models.Review, models.Review.book_id == models.Book.id)
        ...
        all_books = list(
            {book.id: book for book in user_books + reviewed_books}.values()
        )
        return all_books
    ...
    def calculate_user_preferences(self, user_id: int) -> Dict:
        ...
        borrows = (
            ...
            .filter(models.Borrow.user_id == user_id, models.Borrow.is_returned is True)
            .all()
        )
        ...
        for borrow in borrows:
            ...
            days_since_borrow = (
                (borrow.borrow_date - borrow.return_date).days
    ...
    def _get_similarity_scores(
        ...
        for other_user in all_users:
            ...
            other_ratings = {
                ...
                for review in self.db.query(models.Review)
                .filter(models.Review.user_id == other_user.id)
                .all()
            }
            ...
            correlation = self.pearson_correlation(
                user_ratings_list, other_ratings_list
            )
            ...
            if correlation > 0:
                ...
                for book_id in other_book_set - user_book_set:
                    similarity_scores.append(
                        (book_id, correlation * other_ratings[book_id])
                    )
    ...
    def collaborative_filtering(
        ...
        similarity_scores = self._get_similarity_scores(all_users=all_users, user_book_set=user_book_set, user_ratings=user_ratings)
    ...
    def content_based_filtering(
        ...
        for book in user_books:
            ...
            avg_rating = (
                ...
                .filter(models.Review.book_id == book.id)
                .scalar()
                or 3.0
            )
        ...
        all_books = (
            self.db.query(models.Book)
            .filter(models.Book.id.notin_([book.id for book in user_books]))
            .all()
        )
        ...
        for book in all_books:
            ...
            if user_profile:
                score = score / (
                    math.sqrt(sum(w * w for w in user_profile.values())) + 1e-8
                )
    ...
    def hybrid_recommendations(
        ...
        for book_id, score in collab_recs.items():
            combined_scores[book_id] = (
                combined_scores.get(book_id, 0) + score * collab_weight
            )
    ...
    def calculate_confidence_score(
        self, recommendations: List[Tuple[int, float]], user_id: int
    ) -> float:
        """
    ...
    def get_recommendations(
        ...
        # Fetch book details
        books = []
        for book_id, _score in recommended_books:
