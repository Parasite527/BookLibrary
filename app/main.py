...
from . import database, models, schemas, auth, crud, recommendations
...
@app.post("/books/{book_id}/reviews", response_model=schemas.ReviewResponse, status_code=status.HTTP_201_CREATED)
def create_review(
    book_id: int,
...
@app.post("/borrow", response_model=schemas.BorrowResponse, status_code=status.HTTP_201_CREATED)
def borrow_book(
...
def get_my_borrows(
    ...
    for borrow in borrows:
        result.append(schemas.BorrowResponse(
            id=borrow.id,
...
def get_recommendations(
    ...
    if not recommendations_result:
        raise HTTPException(
            status_code=404,
            detail="Not enough data to generate recommendations. Please review or borrow some books first."
        )
